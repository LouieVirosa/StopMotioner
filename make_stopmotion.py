...
def init():
    ...
    return StopMotionCTX(fps=res.fps, src=res.src, dst=res.dst, name=res.output)


if __name__ == "__main__":
    ctx = init()
    print(f"Creating a {ctx.movie_len:.2f} second long "
            f"video from {len(ctx)} images: {ctx.full_name}")
    ctx.generate_clip()
