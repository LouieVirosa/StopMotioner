...
from stopmotion import StopMotionCTX
...
def test_builds_correctly():
    ...
    yellow_print(f"Creating a {ctx.movie_len:.2f} second movie at {os.path.join(dest_dir, filename)}...")
    ctx.generate_clip()
    green_print("Created.")
    ...
    yellow_print(f"Removing created video...")
    assert os.remove(ctx.full_name) == None
    green_print(f"Video file removed.")
